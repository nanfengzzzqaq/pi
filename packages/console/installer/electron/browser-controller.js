...
export class AgentBrowserController {
	...
	trustedEkuaibaoTypeValue(field, value) {
		...
		if (field === "station" && text !== "江苏省南京") {
			throw trustedEkuaibaoError("unsafe_target", "驻地只允许填写江苏省南京");
		}
	...
	trustedEkuaibaoOption(command) {
		...
		if (
			...
			!(/^(?:江苏省南京|(?:南京\s*)?(?:中国\s*\/\s*)?江苏省\s*\/\s*南京)$/.test(value))
		) {
			throw trustedEkuaibaoError("unsafe_target", "驻地候选必须精确指向江苏省南京");
		}
